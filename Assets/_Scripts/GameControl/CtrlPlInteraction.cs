...
{
    private TagGameControl _tagGameCtrl;
    ...
    private void Awake()
    {
        _tagGameCtrl = GetComponent<TagGameControl>();
    ...
    {
        if (_tagGameCtrl.GetTagOwner().IsUnityNull()) return false;
        return player.GetInstanceID() == _tagGameCtrl.GetTagOwner().GetInstanceID();
    ...
    private void TransferTag(GameObject fromPlayer, GameObject toTarget)
    {
        ...
        _tagGameCtrl.SetTagOwner(toTarget);
    }
}
